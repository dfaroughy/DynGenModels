...
class FlowMatching:
	...
	def conditional_probability_path(self):
		...
		"""
		self.mean = (self.t - self.t0) * self.x1 / (self.t1 - self.t0) 
		self.std = ((self.t1 - self.t) + (self.t - self.t0) * self.sigma_min) / (self.t1 - self.t0) 
	
	def conditional_vector_fields(self):
		""" regression objective: conditional vector field u_t(x|x_1)
		"""
		self.u = (self.x1 - (1 - self.sigma_min) * self.x0) / (self.t1 - self.t0) 
	...
	def loss(self, batch):
		...
		u = self.score if self.objective == 'score-matching' else self.u 
